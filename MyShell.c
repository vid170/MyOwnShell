...
int runredirectcommand(char *str,int flag,int total,int curr,int count, char* filen)
{
        char *pch;
        char *myargs[100];
        pch = strtok(str, " ");
...
int runprocess(char *str,char *filen)
{
        ...
        for(int j=0;j<i;j++)
        {
            ...
            {   int p=0,count=0;
                ...
                p=1;
                texit=runredirectcommand(mycommands2[j],1,i,j,p,filen);
            if(texit==2)
            {
                // flag=1;
                return 2;
            ...
            {
                texit=runredirectcommand(mycommands2[j],0,i,j,0,filen);
                if(texit==2)
                return 2;
                if((i-j)==1)
                return texit;
...
{       
        ...
        texit=runprocess(mycommands3[0],filen);
        return texit;
        }
        else
        {
            texit=runprocess(mycommands3[0],"");
            return texit;
        }
        return 0;
}
